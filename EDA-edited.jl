...
using MLJ, Random, Printf, DataFrames, Printf, Plots, MLJLinearModels, CSV, Statistics, HTTP
...
# ╔═╡ 5140ff75-b1fc-48af-aedc-e32c7518feb2
using LinearAlgebra

# ╔═╡ 12dc6fc9-2b5f-468b-8c24-132fb0e59a80
md"""
## Exploratory Data Analysis
First, we start with DS1:
"""
...
begin
	github_url = "https://raw.githubusercontent.com/morten-j/INFO411-Assignment2/main/statlog-heart.data.txt"
	response = HTTP.get(github_url)
	DS1 = (CSV.File(IOBuffer(response.body),header=feature_names)) |> DataFrame
end
...
# ╔═╡ 6bc72707-9e31-4761-85c9-46907f375ee2
md"""
##### 3. Visulization
A number of visualization tools are used to investigate the data structure:
"""

# ╔═╡ fd01cfb6-3f6e-4704-870a-4a5580c68444
md"""
###### 3.1. scatter plot
"""

# ╔═╡ e52a4a36-e5a2-4c1a-9df7-acc4b732a83c
begin
	scatter_plots = [scatter(DS1[:,i], xlabel="$(feature_names[i])", ylabel="amount", legend=false) for i in 1:length(feature_names)] 
	plot(scatter_plots..., layout = (4,4), size=(1000, 1000))
end

# ╔═╡ 239f2941-bc73-4df5-98bf-7f15f6eb678a
md"""
**Insights**:
- There are both categorical and continuous variables. 
- Some potential outliers can bee seen. 
- Some subcategories seem to include very low number of values. 
- let's check the distributions.
"""

# ╔═╡ 347e6740-8725-48a2-a4a2-76ad6a0836c0
md"""
###### 3.2. Histograms/Barplots
The data is first splitted into separate discrete and continuous subsets. 
"""

# ╔═╡ d5d6a8b9-18f4-46f9-b384-21ff2bddf6ad
function separate(data)
	# Define a threshold for the number of unique values to consider an attribute as categorical
...
# ╔═╡ 7d3a7b71-ba6a-4e39-a3bc-26e722996e3b
cat1 , con1 = separate(DS1);

# ╔═╡ 2ba5c0c5-9730-409c-82ea-00e8b62b51e4
begin
	cbar_plots = [histogram(con1[:,i], xlabel="$(names(con1)[i])", ylabel="amount", legend=false) for i in 1:length(names(con1))] 
	plot(cbar_plots..., layout = (5), size=(1000, 1000), )
end

# ╔═╡ c0ea870b-9b0d-4280-a6ba-4c01dfd48ab5
...
# ╔═╡ a136150b-09bc-4b65-971f-134077dda6fc
cat11 = cat_float_to_int(cat1);

# ╔═╡ b7dac874-1384-478f-a74f-5dc55ecec049
begin
	using StatsBase
	bp = repeat([plot()], 9)
	for col in 1:length(names(cat11))
		...
		bp[col]=bar((x -> datamap[x]).(s), xticks=(1:length(s), s), label=names(cat11)[col])
	end
	plot(bp..., layout=(5,2), size=(1000,1000))
end

# ╔═╡ da8c2be8-7d76-4ee8-823e-1bf23cbaadd7
describe(DS1)

# ╔═╡ b53cadeb-452e-4c2e-b947-8dbe41cfd072
md"""
**Insights**:
- Continuous variables are approximately normally distributed, except for oldpeak. 
- They differ in range, so we should consider standardization.
- Some categorical variables have subcategories with low number of observations.
- We may allocate these observations to the other sub-categories based on the definitions. But, let's wait to see the DS2. 
"""

# ╔═╡ 8203ccb6-8f9a-4667-98f2-73b9ec53b7de
md"""
##### 4. Correlations
Checking the correlation between features and also label. 
"""

# ╔═╡ 093d897a-ea47-4627-ac55-e2c3fc2ee681
begin
	correlation_matrix = cor(Matrix(DS1))
	cmin, cmax = -1.0, 1.0  # You can adjust these values as needed
	hm = heatmap(correlation_matrix, 
   			xticks=(1:size(correlation_matrix, 1), names(DS1)),
    		yticks=(1:size(correlation_matrix, 2), names(DS1)),
	        # xticklabels=names(X)[1:end-1], 
	        # yticklabels=names(X)[1:end-1], 
	        color=:coolwarm,
	        aspect_ratio=:equal,
			clims= (cmin, cmax),
	        xlabel="Features",
...
md"""
**Insights**:
- heart disease have a moderate negative correlation with maximum heart rate achieved and is positively correlated with defect type (thal). 
- fasting blood sugar(fbs) is weakly correlated with disease diagnosis.
- slope and oldpeak features are highly correlated. so, models may use one instead of both. 
"""

# ╔═╡ f946b6dc-5d26-472a-b422-4e08bc98b86f
md"""
##### 5. Clustering 
clustering with different combination of attributes using kmeans. 
"""

# ╔═╡ 8fe48e8e-07ea-4b15-b7f1-318b9ff53540
begin
	coerce!(DS1, :num => OrderedFactor{2});
    y, X = unpack(DS1, ==(:num),name->true);
end

# ╔═╡ 883c63b1-911e-4abb-97bd-c251c46d98cb
begin
	# Select the attributes you want to use for clustering
	attributes_for_clustering = select(con1, [:1, :4])
...
# ╔═╡ 37e4ba5e-67ec-431f-919a-143313229982
begin
	scatter(con1.chol, con1.thalach, 
	    group=assignments,  # Assignments from k-means clustering
	    xlabel="chol",
	    ylabel="thalach",
	    # zlabel="cp",
	    legend=true,
...
# ╔═╡ dbe96d61-ef75-4c1a-a3e5-9820d891680f
md"""
**Insights**:
- 
"""

# ╔═╡ 1d35cda3-aed7-4f8d-86a3-5d8ae3eec483
md"""
#### EDA - DS2
"""
...
begin
	locs = ["processed.hungarian.data", "processed.switzerland.data","processed.va.data", "processed.cleveland.data"]
	git_url = "https://raw.githubusercontent.com/morten-j/INFO411-Assignment2/main/"
	# response = HTTP.get(github_url+locs[1])
	DS2H = (CSV.File(IOBuffer(HTTP.get(git_url*locs[1]).body),header=feature_names)) |> DataFrame
	DS2S = (CSV.File(IOBuffer(HTTP.get(git_url*locs[2]).body),header=feature_names)) |> DataFrame
	DS2V = (CSV.File(IOBuffer(HTTP.get(git_url*locs[3]).body),header=feature_names)) |> DataFrame
	DS2C = (CSV.File(IOBuffer(HTTP.get(git_url*locs[4]).body),header=feature_names)) |> DataFrame
end
...
md"""
**Insights**:
- Missing values are encoded as ? character. 
- Numerical values are coded as strings in some columns.
...
md"""
Note: 
- The first 3 datasets are concatenated to create DS2 train set.
- Fourth dataset (Cleveland) is saved as test set. 
""" 

# ╔═╡ 27aca0b8-0ed4-4e48-afda-f5fce5a55469
begin 
	DS2Hn = convert_to_float(DS2H)
	DS2Sn = convert_to_float(DS2S)
	DS2Vn = convert_to_float(DS2V)
	DS2Cn = convert_to_float(DS2C)
	DS2n = vcat(DS2Hn, DS2Sn, DS2Vn)
end

# ╔═╡ f325deb3-13e2-47d8-847d-719eb44ff533
md"""
**Insights**:
- Some columns seem to include many missing values.

"""

# ╔═╡ 70969e72-6778-4e6d-b427-c4f7de44f64b
md"""
##### 3.Imputation
As we will use typical imputation (versus more complex methods such as MI), we will discard columns with more that 15% missing values (a bit higher than 10% suggested in literature). 
"""

# ╔═╡ ee009140-345f-4abe-a0d3-e42311ef5db1
function discard_high_missing_cols(fdata)
	...
	QC_fdata = copy(fdata)
	del_cols = []
	df10per = (15*size(QC_fdata)[1])/100
...
# ╔═╡ 4377b0ea-03ce-465b-a92b-8763a7ad7776
begin 
	QC_DS2Hn, HHM = discard_high_missing_cols(DS2Hn)
	QC_DS2Sn, SHM  = discard_high_missing_cols(DS2Sn)
	QC_DS2Vn, VHM= discard_high_missing_cols(DS2Vn)
	QC_DS2Cn, CHM = discard_high_missing_cols(DS2Cn)
	QC_DS2n, HM = discard_high_missing_cols(DS2n)
	println(HHM, "\n", SHM, "\n" , VHM, "\n", CHM, "\n", HM)
end

# ╔═╡ 9fa1202a-28bd-4322-82a1-e28d95fa3d3f
md"""
**Insights**:
- The va dataset have many columns with missing values. 
- However, the combined QC_DS2n dataset have three columns with more than 15% missing values which are dicarded now. 
"""

# ╔═╡ 02c7eb02-475c-448a-bdc9-82ab5143a872
function imputing(qcdata)
	impmat = Impute.knn(
	         convert(Matrix{Union{Missing, Float64}}, Matrix(qcdata)),
	         dims=:cols)
	impmat_float = coalesce.(impmat, 999.999)
	imp_df = DataFrame(impmat, Symbol.(names(qcdata)))
	imp_fdf = DataFrame(impmat_float, Symbol.(names(qcdata)))
	return imp_df , imp_fdf
end

# ╔═╡ ae3934fc-ed7e-4484-8421-bc65bd3e5cb2
begin 
	imp_QC_DS2n, imp_QC_flDS2n = imputing(QC_DS2n);
	imp_DS2n, imp_flDS2n = imputing(DS2n);
	# imp_DS2Hn, imp_flDS2Hn = imputing(QC_DS2Hn)
	# imp_DS2Sn, imp_flDS2Hn = imputing(QC_DS2Sn)
	# imp_DS2Vn, imp_flDS2Hn = imputing(QC_DS2Vn)
	imp_QC_DS2Cn, imp_QC_flDS2Cn = imputing(QC_DS2Cn);
end

# ╔═╡ 0f01b437-0b8d-4f62-b655-134eed5b3bc4
describe(imp_QC_flDS2n)

# ╔═╡ b7d8aad1-3bfb-4696-9831-7dc27d1ab05f
begin
	feature_names2 = names(imp_QC_flDS2n)
	scatter_plots2 = [scatter(imp_QC_flDS2n[:,i], xlabel="$(feature_names2[i])", ylabel="amount", legend=false) for i in 1:length(feature_names2)] 
	plot(scatter_plots2..., layout = (4,4), size=(1000, 1000))
end

# ╔═╡ e45c298f-0aab-4120-8966-61f5f16f0fd1
cat2 , con2 = separate(imp_QC_flDS2n);

# ╔═╡ 2c68afe1-6ba3-4e82-a629-c3b997face7a
begin
	cbar_plots2 = [histogram(con2[:,i], xlabel="$(names(con2)[i])", ylabel="amount", legend=false) for i in 1:length(names(con2))] 
	plot(cbar_plots2..., layout = (5), size=(1000, 1000), )
end

# ╔═╡ 6b4165de-278f-4bd5-889d-a232c9db727d
function std_encode(imp_qc_fl, f)
	std_imp_qc_fl = copy(imp_qc_fl)
	if f == 1
		std_imp_qc_fl.num = ifelse.(std_imp_qc_fl.num .== 0, 0, 1)
	end
	std_imp_qc_fl.cholcat = ifelse.(std_imp_qc_fl.chol .== 0, 0.0, 1.0)
	# mean_chol = mean(std_imp_QC_flDS2n.chol[std_imp_QC_flDS2n.chol .!= 0])
	# std_imp_QC_flDS2n.chol .= ifelse.(std_imp_QC_flDS2n.chol .== 0, mean_chol, std_imp_QC_flDS2n.chol)
...
# ╔═╡ 556f7a0d-28db-4537-8ad4-8dcb0a887cf6
begin
	std_imp_QC_flDS2nb = std_encode(imp_QC_flDS2n, 1)
	std_imp_QC_flDS2n = std_encode(imp_QC_flDS2n, 0)
	std_imp_QC_flDC2nb = std_encode(imp_QC_flDS2Cn, 1)
	std_imp_QC_flDC2n = std_encode(imp_QC_flDS2Cn, 0)
	select!(std_imp_QC_flDC2nb, Not(HM)) 
	select!(std_imp_QC_flDC2n, Not(HM)) 

end

# ╔═╡ c86eeaad-e4f9-4e86-8e25-f9d303d229e2
describe(std_imp_QC_flDC2n)

# ╔═╡ ef65e12c-e107-4289-bfc8-34897027b138
cat2s , con2s = separate(std_imp_QC_flDS2n)

# ╔═╡ 1d140eb2-8df5-4bc7-8e82-ad4f852c27f2
begin
	cbar_plots2s = [histogram(con2s[:,i], xlabel="$(names(con2s)[i])", ylabel="amount", legend=false) for i in 1:length(names(con2s))] 
	plot(cbar_plots2s..., layout = (5), size=(1000, 1000), )
end

# ╔═╡ 631801d4-c933-47b1-a2b8-5219e1fdb096
begin 
	cat22 = cat_float_to_int(cat2s)
	bp2 = repeat([plot()], size(cat2s)[2])
	for col in 1:length(names(cat22))
		s = sort(unique(cat22[:,col]))
		datamap = countmap(cat22[:,col])
		bp2[col]=bar((x -> datamap[x]).(s), xticks=(1:length(s), s), label=names(cat22)[col])
	end
	plot(bp2..., layout=(4,2), size=(1000,1000))
end

# ╔═╡ b5946684-6336-4cfe-b0cc-cae73776ea85
begin
	coerce!(std_imp_QC_flDS2nb, :num => OrderedFactor{2});
    y2, X2 = unpack(std_imp_QC_flDS2nb, ==(:num),name->true);
end

# ╔═╡ 82bb3c1f-d8b8-43e7-b358-5b26a51268ec
begin
	correlation_matrix2 = cor(Matrix(std_imp_QC_flDS2n))
	# cmin, cmax = -1.0, 1.0  # You can adjust these values as needed
	hm2 = heatmap(correlation_matrix2, 
   			xticks=(1:size(correlation_matrix2, 1), names(std_imp_QC_flDS2n)),
    		yticks=(1:size(correlation_matrix2, 2), names(std_imp_QC_flDS2n)),
	        # xticklabels=names(X)[1:end-1], 
	        # yticklabels=names(X)[1:end-1], 
	        color=:coolwarm,
	        aspect_ratio=:equal,
	        ...
	        ylabel="Features",
	        title="Correlation Matrix", size = (900,900), fontsize= 20)
	for i in 1:size(correlation_matrix2, 1)
	    for j in 1:size(correlation_matrix2, 2)
	        annotate!(i , j , text(round(correlation_matrix2[i, j], digits=2), 10, :white, :center))
    	end
	end
	hm2
end

# ╔═╡ 4672fc39-7f41-44f6-b7eb-d7eceda32e9d
begin
	# Select the attributes you want to use for clustering
	attributes_for_clustering2 = select(con2s, [:chol, :thalach])
	# Convert the DataFrame to a matrix
	data_matrix2 = Matrix(attributes_for_clustering2)
	k2 = 2
	# Perform K-means clustering
	result2 = kmeans(data_matrix2', k2)
	# Access cluster assignments and centroids
	assignments2 = result2.assignments
	centroids2 = result2.centers
end

# ╔═╡ 86af6ad5-99b1-4078-8b9c-cb26988b17d6
begin
	scatter(con2s.chol, con2s.thalach, 
	    group=assignments2,  # Assignments from k-means clustering
	    xlabel="chol",
	    ylabel="thalach",
	    # zlabel="cp",
	    legend=true,
...
# ╔═╡ e25705e3-24c3-4f47-a5e8-e743533f6aac
begin 
