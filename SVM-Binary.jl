...
# ╔═╡ 44a8e6b0-603f-11ee-36a1-9799830515ae
using MLJ, Random, Printf, DataFrames, Printf, Plots, MLJLinearModels, CSV, Statistics, HTTP, MLJLIBSVMInterface, LIBSVM,PlutoUI, HypertextLiteral, StatsPlots, PyPlot, MLBase

# ╔═╡ 6f052d09-291c-49d8-87d8-89e8428b471e
md"""
## Modelling - SVM
 ###### packages:
"""

# ╔═╡ 6f9d84b7-7c21-4e4e-95e4-55078f0f7fd3
md"""
 ###### Functions:
"""

# ╔═╡ 3edfa3d5-def2-490c-a0bf-8cb86d35bc21
md"""
##### 1. Loading and prepration
Here, we will use the dataset with binary labels.
"""

# ╔═╡ e6ecc186-8dd9-4fd6-a806-af7012d0e043
begin
	github_tr_url = "https://raw.githubusercontent.com/morten-j/INFO411-Assignment2/main/Data/bin_DS2.csv"
	github_te_url = "https://raw.githubusercontent.com/morten-j/INFO411-Assignment2/main/Data/bin_DS2_test.csv"
	...
	test_data = (CSV.File(IOBuffer(response_te.body))) |> DataFrame; nothing
end

# ╔═╡ d1fdfb2e-46f7-44c0-bf26-30600e38b5d4
md"""
Convert and code X and y to fit model requirement:
"""

# ╔═╡ 465a8794-8bca-4a48-9c7d-25180d74dd06
begin
	...
	cy = categorical(y)
	cyt = categorical(yt); nothing
end

# ╔═╡ b5e0b455-4258-4b55-b44f-cb89426a1b1e
md"""
 ##### 2. Initiate SVM model
"""

# ╔═╡ 4972f30e-4874-4b08-95d2-c779e1b8e56a
SVC = @load SVC pkg=LIBSVM;

# ╔═╡ 54639f5b-a9f0-42b6-a5a9-9e9e7263a8d1
function inter_plot1(cost, degree)
        T4a_accu_tr = []
        T4a_accu_te = []
        for deg in 1:degree
			for co in cost
	            svc = SVC(kernel=LIBSVM.Kernel.Polynomial, degree=Int32(deg), 					cost=cost)
	            svc_mach = machine(svc, Xc, cy)
	            MLJ.fit!(svc_mach, verbosity=0)
	            push!(T4a_accu_tr, accuracy(MLJ.predict(svc_mach, Xc), cy))
	            push!(T4a_accu_te, accuracy(MLJ.predict(svc_mach, Xtc), cyt))
			end
        end
        Plots.plot(1:degree, T4a_accu_tr, label="Training", marker=:c, 							xlabel="Degree", ylabel="Accuracy", legend=:top)
	    y_ticks = 0.0:0.1:1.0 
        plot!(1:degree, T4a_accu_te, label="Testing", marker=:d, yticks=y_ticks)
    end

# ╔═╡ b5fb91cb-982c-42af-86d5-e1cbb16e81fc
md"""
 ##### 3. primary model visual inspection
Let's fit the model to train data and examine its performance on train and test across different parameter values. 
We will use this insight to decide parameter range used in automatic tunning. 
"""

# ╔═╡ 59524d98-72c6-454d-b648-8e7b2a37624c
md"""
**degree:**
"""

# ╔═╡ fed4870a-c7a7-4fcd-bb9e-45e2aaf33268
@bind degree Slider(1:10, default=1, show_value=true)

# ╔═╡ 14da5230-c9e5-415d-a76d-1359fff19f9a
md"""
**cost:**
"""

# ╔═╡ 1711387d-eb7e-47f8-ba7f-e8105224eaa4
begin
    @bind cost Slider(0.1:0.1:5.0, default=0.1, show_value=true)
end

# ╔═╡ f6358f93-ee53-4890-b727-206327bbc878
inter_plot1(cost, degree)

# ╔═╡ b75aaa0c-845e-4f58-b3f2-89c7d63250eb
md"""
##### 4. Automatic tuning:
"""

# ╔═╡ f26f219f-baa9-4547-83e1-b4e6df0d4505
begin
	svc_model = SVC(kernel=LIBSVM.Kernel.Polynomial)
	params(svc_model) 
	Random.seed!(1)
	r1 = range(svc_model, :degree, lower=1, upper=5)
	r2 = range(svc_model, :cost, lower=0.1, upper=1.0)
	# r3= range(svc_model, :tolerance, lower=0.001, upper=0.1)
	# r4= range(svc_model, :gamma, lower=0.0, upper=0.5)
	tm = TunedModel(model=svc_model, tuning=Grid(resolution=5),
...
# ╔═╡ aa485793-dee5-493a-bb0d-a5e89bc677c6
rep = report(m)

# ╔═╡ cdc9b3ff-be34-4ac7-a420-baacf8323fae
md"""
##### 5. Visualization
Performance across runs:
"""

# ╔═╡ e8922f9b-0893-4dd7-9c91-6713614b4038
begin 
	vv = violin()
	for (i,run) in enumerate(rep.history)
		violin!(["run=$i"], run.per_fold, label=nothing)
	end
	vv
end

# ╔═╡ 68aaaaa5-8c48-465f-972d-a8b1c43ce80d
md"""
Performance across parameters range
"""

# ╔═╡ eceaf8d6-b90e-47a5-9be1-7cda6504122d
begin 
	res = rep.plotting
...
# ╔═╡ ffa5c569-95fb-4499-896f-37ad7a111f3a
md"""
**Insights**:
- Best accuracy is given by cost of ~(0.7-0.8) and degree of 2. 
- The best model from automatic tuning has cost = 0.775 and degree = 2. 
"""

# ╔═╡ b49fd570-8c12-4c8c-b4d8-d2e3bb87511b
md"""
##### 6. Prediction Performance 
"""

# ╔═╡ ec4a4dc3-30f8-4528-81db-8fee7f63b219
begin
	yhat = MLJ.predict(m, Xtc)
	test_ac = accuracy(yhat, cyt)
	ytehat = MLJ.predict(m, Xc)  
	train_ac = accuracy(ytehat, cy)
	test_mcrate = misclassification_rate(yhat, cyt)
	train_mcrate = misclassification_rate(ytehat, cy)
	println("best model train accuracy:\n", train_ac, "\nbest model test accuracy:\n", test_ac, "\nbest model train misclassification rate:\n", train_mcrate,"\nbest model test misclassification rate:\n", test_mcrate)
end
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "4c61d2fa279ebab5c525a303241a25bb61684f4f"
