...
begin
	...
	mach_ri = machine(reg_ri, Xtr, ytr)
	fit!(mach_ri)
	coef = []
	fp_ri = fitted_params(mach_ri)
	coefs_ri = fp_ri.coefs
	intercept = fp_ri.intercept
	for (name, val) in coefs_ri
	    println("$(rpad(name, 8)):  $(round(val, sigdigits=3))")
		push!(coef, val)
	end
end
...
begin
	values_list = values(coefs_ri)
	values_array = collect(values_list)
...
begin
	...
	perf1 = evaluate(reg_ri1, Xtr, ytr, resampling=cv1, measure=rms, verbosity=0)
	ten_perf_rig = perf1.per_fold
	for i in ten_perf_rig[1]
		push!(rms_scores2, i)
	end
	println(rms_scores2)
end
...
md"""
### Viloin Plot of multivariate regression and ridge regression 
"""

# ╔═╡ 96974e3e-b874-4b6a-9f28-679e82b243f4
begin
    violin(["Score comparison"], rms_scores1, side=:left, label="t1")
...
mean_list = Float64[]
	for i in 2:20
		fold_list = []
		for j in 1:10
	    	cv2 = CV(nfolds=i, shuffle=true)
	    	perf2 = evaluate(reg_lr, Xtr, ytr, resampling=cv2, measure=rms, verbosity=0)
			push!(fold_list, mean(perf2.per_fold))
		end
...
mean_list2 = Float64[]
	for i in 2:20
		...
		for j in 1:10
	    	cv2 = CV(nfolds=i, shuffle=true)
	    	perf2 = evaluate(reg_ri1, Xtr, ytr, resampling=cv2, measure=rms, verbosity=0)
			push!(fold_list, mean(perf2.per_fold))
		end
		push!(mean_list2, mean(fold_list[1][i]))
	end
	# min of cross validation
...
git-tree-sha1 = "779527a9e0d0979ff6804f39721ed7afa52930c6"
