...
using MLJ, Random, Printf, DataFrames, Printf, Plots, CSV, Statistics

# ╔═╡ 9254a080-8be4-45bf-88d9-b2de6c3727d7
begin
	using HTTP
	github_username = "morten-j"
	repository_name = " INFO411-Assignment2"
	train_file_path = "bin_DS2.csv"
	test_file_path = "bin_DS2_test.csv"
	# https://github.com/morten-j/INFO411-Assignment2/blob/main/bin_DS2_test.csv
...
# ╔═╡ dab28a62-d9cb-4792-afe3-eb491289b452
using MLJModels, MLBase

# ╔═╡ 368c0c7a-72fe-4b09-b221-f8d4184756ab
# Import sklearn.metrics to julia
using PyCall

# ╔═╡ 4b1dbc10-fd65-4e41-9dc8-f340cb08446e
using  PlutoUI, HypertextLiteral

# ╔═╡ 5856fb6c-f9e7-436f-aff2-2a9ca7e638eb
begin
	# data = coerce(DS2, autotype(DS2, :discrete_to_continuous));
	...
	# train, test = partition(eachindex(y2), 0.8, shuffle=true, rng=13);
	Xc = coerce(X, autotype(X, :discrete_to_continuous));
	Xtc = coerce(Xt, autotype(Xt, :discrete_to_continuous));
	cy = categorical(y)
	cyt = categorical(yt)
	# Xtr = Array{Float32}(Xc)
	# Xte = Array{Float32}(X2[test,:])
	# Ytr = Array{Int32}(y2[train,:])
	# Yte = Array{Int32}(y2[test,:])
	# tX2 = selectcols(X2, :) 
	# xall = Matrix(data)'
	# xtr = xall[:,train]
	# ytr = cy[train]
...
# ╔═╡ e91280e2-a721-4844-9b43-efb638e82871
begin
	using XGBoost
	bst = Booster((X, cy), max_depth=4, η=0.1, objective = "binary:logistic")
	# 20 rounds of training
	update!(bst, (X, cy), num_round=20)
	ŷ = XGBoost.predict(bst, X)
end

# ╔═╡ d62231cf-4b5a-4efb-a927-ab88196b4a3d
params(bst) 

# ╔═╡ 3158b529-0354-4403-86c3-36b4f1fba2ef
ŷte = XGBoost.predict(bst, Xtc)

# ╔═╡ 3fe1874d-b28a-4850-bb31-257b017ac0a4
size(cyt)

# ╔═╡ a7f13aa4-24f5-4a67-9a5f-9f16a0bd1c06
y_binary_decision = ifelse.(ŷte .>= 0.5, 1, 0)

# ╔═╡ 5169d1ce-92e9-4e41-8f69-43cebd2ecb34
accuracy(y_binary_decision, cyt)

# ╔═╡ 80205f3d-57df-47f2-818f-43113f7b2b41
yte_binary_decision = ifelse.(ŷ .>= 0.5, 1, 0)

# ╔═╡ 88582d03-d72b-4e49-b16f-ddf1001c28f6
accuracy(yte_binary_decision, cy)

# ╔═╡ 964edaec-1dab-49b2-a2c8-a792c2299cc6
trees(bst; with_stats=true)

# ╔═╡ 987fe47e-3c58-462a-acc5-c0ff3d95312d
begin
...
# ╔═╡ 6dd3eedd-024f-49db-8a80-2eb3aa828842


# ╔═╡ 66dee24f-1439-4d08-913a-599f4f14f9a5
imp = DataFrame(importancetable(bst))

# ╔═╡ c857184d-cbea-49c1-a947-e17c18629cb1
begin
	
	column_names = names(imp)[2:4]

	# Create a stacked line plot
	plot()
	for col in column_names
	    plot!(imp.feature, imp[!, col], label=col, linewidth=2)
	end
	
	xlabel!("x")
	ylabel!("Value")
	
	# Remove default legend and show the custom legend
	plot!(legend=false)
	plot!(legend=:right)
end

# ╔═╡ 17257ba9-0007-4e40-8ed1-0ff462b0c58e
@bind pars Select(column_names)
