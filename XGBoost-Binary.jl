...
# ╔═╡ fb154ab6-6347-11ee-18d8-3d30e91e514a
using MLJ, Random, Printf, DataFrames, Printf, Plots, CSV, Statistics, HTTP, XGBoost, MLJModels, MLBase, PyCall, PlutoUI, HypertextLiteral

# ╔═╡ 65a5a240-43ad-4d1a-b3e2-3566ac148854
md"""
## Modelling - XGBOOST
 ###### packages:
"""

# ╔═╡ 4f2f182a-5f96-4f92-88af-55a56a7dc02c
md"""
###### Functions:
"""

# ╔═╡ 38a18792-c9d0-4109-8759-6aeaf9afdcaa
md"""
##### 1. Loading and prepration
Here, we will use the dataset with binary labels.
"""

# ╔═╡ 9254a080-8be4-45bf-88d9-b2de6c3727d7
begin
...
# ╔═╡ 5856fb6c-f9e7-436f-aff2-2a9ca7e638eb
begin
...
# ╔═╡ a167d806-bde1-4caa-8eb2-598e8b36872a
function tune_predict_plot(X, cy, d, m, r)
	#initiate model
	bst = Booster((X, cy), max_depth=d, η=m, objective = "binary:logistic")
	#update
	update!(bst, (X, cy), num_round=r)
	#predict
	ŷ = XGBoost.predict(bst, X)
	ŷte = XGBoost.predict(bst, Xtc)
	#plot roc
	sklearn = pyimport("sklearn.metrics")
	fpr, tpr, thresholds = sklearn.roc_curve(Array{Int64}(cyt), ŷte)
	fpr2, tpr2, thresholds2 = sklearn.roc_curve(Array{Int64}(cy), ŷ)
	roc_p = plot(fpr, tpr, label = "test")
	plot!(fpr2, tpr2, label = "train")
	title!("ROC curve")
	
	return bst, roc_p
end

# ╔═╡ f58aaa52-227c-4624-b70a-b732d947c978
md"""
 ##### 2. Tuning
Runs a function that fits the xgboost model to train data and generates roc curves for different values of model parameters.
"""

# ╔═╡ 3bd7dea1-fd8c-48d2-8247-917967d79962
md"""
**choose maximum depth:**
"""

# ╔═╡ 89044432-e049-49c3-9489-8ccfbbce58b7
@bind max_dep Slider(1:10, default=1, show_value=true)

# ╔═╡ 4ce11993-3936-40c8-a275-85d91d61824f
md"""
**choose learning rate:**
"""

# ╔═╡ bf6f1384-fe99-431d-9f90-6d4af081e5f6
@bind eta Slider(0:0.1:1, default=0.1, show_value=true)

# ╔═╡ 1d7d6196-ba70-4f9b-9188-fbb7dd6d8eba
begin
	bst, roc_plot = tune_predict_plot(X, cy,max_dep , eta, 20); nothing
	roc_plot
end

# ╔═╡ 46cd88c2-ff9d-459e-8cad-4b7a1a03460f
md"""
 ##### 3. Best model performance
"""

# ╔═╡ 79e66cf3-a1a8-4e45-85f1-a0e9a6cda25c
begin
	best_bst, best_roc = tune_predict_plot(X, cy, 4 , 0.1, 20); nothing
	best_roc
end

# ╔═╡ 0f0bb200-81cf-4109-aafd-4ff56483b832
begin
	ŷ = XGBoost.predict(best_bst, X)
	ŷte = XGBoost.predict(best_bst, Xtc)
	y_binary_decision = ifelse.(ŷte .>= 0.5, 1, 0)
	yte_binary_decision = ifelse.(ŷ .>= 0.5, 1, 0)
	test_ac = accuracy(y_binary_decision, cyt)
	train_ac = accuracy(yte_binary_decision, cy)
	println(" model train accuracy:\n", train_ac, "\n model test accuracy:\n", test_ac)
end

# ╔═╡ 1de9b1d6-33f1-4035-8406-c572031ae86c
md"""
 ##### 4. Feature importance metrics
"""

# ╔═╡ 66dee24f-1439-4d08-913a-599f4f14f9a5
begin
	imp = DataFrame(importancetable(best_bst))
	column_names = names(imp)[[3,5,6]]
end

# ╔═╡ ac497531-bf25-4f20-b40b-c9abbc9cacac
imp

# ╔═╡ 3205984d-146d-4050-8630-e3a0b935e43e
md"""
**Choose a metric:**
"""

# ╔═╡ 17257ba9-0007-4e40-8ed1-0ff462b0c58e
@bind pars Select(column_names , default= "none")

# ╔═╡ 57734456-71ac-450b-b7d9-5136874319e0
begin
	plot(imp.feature, imp[!, pars], label=pars, linewidth=2)
end

# ╔═╡ bba55c70-10bb-41a3-961b-d18fd324413b
md"""
**Insights**:
- chol, age and thalach are most commonly used for splitting.
- cp and chol contribute most to reducing prediction error. 
- cp and chol, age and exang influence a larger portion of the dataset.
"""
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "d4bbbb2731516e65f7f60d1ada4711b20fbc6303"
...
[[deps.XGBoost]]
deps = ["AbstractTrees", "CEnum", "CUDA", "JSON3", "LinearAlgebra", "OrderedCollections", "SparseArrays", "SparseMatricesCSR", "Statistics", "Tables", "Term", "XGBoost_jll"]
